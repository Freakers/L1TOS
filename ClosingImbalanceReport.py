    ...
    def __init__(self):
        ...
        for record in file:
            if ".TO" in record:
                #print(record)
                fields = record.split(",")
                l1_symbols[str(rec_count)] = fields[6].split("=").pop(1) + ";" + \
        ...
        for key, l1_symbols in l1_symbols.items():
            ...
            if "None" not in ltp:
                if side == "B":
                    last_trade_price = float(tos.get_last_trade(l1_symbols.split(";").pop(0)).__str__().split(";").pop(0))
...
class TOSFileReader:
    ...
    def __init__(self):
        ...
        for record in file:
            if ".TO" in record:
                #print(record)
                self.tos_records[str(rec_count)] = record
                rec_count = rec_count + 1
        ...
        for key, tos_record in self.tos_records.items():
            if symbol in tos_record:
                ...
                for item in tos_record.split(','):
                    ...
                    self.last_print = message_dict.get('Price')
                if datetime(n.year, n.month, n.day, int(mtime[0]), int(mtime[1]),
                    ...
                    # print("Close Trade Key: " + key.__str__())
                    # print("Close Trade Matched: " + str(tos_record))

                    # #tos_record.
                    self.last_trade_record[key] = tos_record
                    toslastprice = tos_record.split(",").pop(5).split("=").pop(1)
                    if last_print.split(",").__len__() > 0:
                        print("Last Print         : " + str(last_print))
                        flds = last_print.split(",")
                    if flds.__len__() > 0:
                        print(flds.__str__())
                    return toslastprice + "; 1000.00"
                    break
                else:
                    last_print = tos_record
