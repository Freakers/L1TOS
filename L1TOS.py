...
class Orders:
    ...
    def __init__(self, feed_type="5556", region="1"):
...
class Symbols:
    ...
    def registersymbols(self):
        ...
        for record, symbol in self.symbols.items():
            print(symbol)
            t = threading.Thread(target=self.registersymbol, args=(symbol, "L1", "5556",))
            t.start()
            t = threading.Thread(target=self.registersymbol, args=(symbol, "L2", "5556",))
            t.start()
            t = threading.Thread(target=self.registersymbol, args=(symbol, "TOS", "5556",))
            t.start()
...
reactor.listenUDP(5556, ppro_datagram(str(sys.argv[1])))
reactor.run()
